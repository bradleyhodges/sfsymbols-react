...
export interface PreviewIcon {
    name: string;
    viewBox: string;
    paths: readonly { d: string; fill?: string; fillOpacity?: number }[];
}
type Value =
    | null
    | string
        ...
        do {
            for (let parameter = 0; parameter < arity[command]; parameter++) {
                ...
                if (command === "A" && (parameter === 3 || parameter === 4)) {
                    if (source[offset] !== "0" && source[offset] !== "1")
                        return false;
                    offset++;
                } else {
...
export function parseIcon(source: string, name: string): PreviewIcon {
    if (
        ...
        Buffer.byteLength(source) > MAX_LEAF_BYTES
    )
        throw new Error("Icon exceeds supported limits.");
    ...
    function declaration(node: Node) {
        if (
            ...
            throw new Error("Only static const declarations are supported.");
        for (const item of node.declarations as Node[]) {
            ...
            if (
                ...
                !record(item.init) ||
                bindings.has(id.name)
            )
                throw new Error("Invalid static binding.");
            bindings.set(id.name, item.init as Node);
        }
    }
    for (const statement of program.body) {
        if (statement.type === "VariableDeclaration")
            declaration(statement as unknown as Node);
        else if (
            ...
            throw new Error("Executable icon modules are not supported.");
    }
    let count = 0;
    const visiting = new Set<string>();
    const memo = new Map<string, Value>();
    function binding(key: string, depth: number): Value {
        if (visiting.has(key) || !bindings.has(key))
            throw new Error("Invalid static reference.");
    ...
    function evaluate(node: Node, depth: number): Value {
        if (++count > 10000 || depth > 32)
            throw new Error("Static data exceeds supported limits.");
        ...
        if (node.type === "Identifier" && typeof node.name === "string")
            return binding(node.name, depth);
        ...
        if (
            ...
            for (const item of node.properties as Node[]) {
                ...
                if (
                    typeof text !== "string" ||
                    ["__proto__", "constructor", "prototype"].includes(text) ||
    ...
    if (!icon.svgPathData.length || icon.svgPathData.length > 128)
    ...
    const paths = icon.svgPathData.map((item) => {
        if (
            ...
            !validGeometry(item.d)
        )
            throw new Error("Invalid path geometry.");
        geometryLength += item.d.length;
        if (geometryLength > MAX_LEAF_BYTES)
            throw new Error("Icon geometry exceeds supported limits.");
        if (
            item.fill !== undefined &&
            item.fill !== "currentColor" &&
            item.fill !== "none"
        )
            throw new Error("Unsupported path fill.");
        if (
            ...
            throw new Error("Invalid path opacity.");
        return {
            d: item.d,
            fill: item.fill as string | undefined,
            fillOpacity: item.fillOpacity as number | undefined,
        };
    });
