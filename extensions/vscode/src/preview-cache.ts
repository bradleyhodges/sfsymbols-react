...
import { renderSvg, type PreviewIcon } from './static-data';
...
export class PreviewCache {
  ...
  constructor(storage: vscode.Uri) {
    this.directory = vscode.Uri.joinPath(storage, `previews-${randomUUID()}`);
  }
  async image(icon: PreviewIcon): Promise<vscode.Uri> {
    if (this.disposed || this.pending >= 16) throw new Error('Preview queue is busy.');
    const dark = vscode.window.activeColorTheme.kind === vscode.ColorThemeKind.Dark || vscode.window.activeColorTheme.kind === vscode.ColorThemeKind.HighContrast;
  ...
  async close(): Promise<void> {
    this.disposed = true;
    await this.queue;
    try { await vscode.workspace.fs.delete(this.directory, { recursive: true }); }
