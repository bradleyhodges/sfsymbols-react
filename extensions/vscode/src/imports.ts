...
function syntax(source: string, language: string): Tree {
    if (Buffer.byteLength(source) > 1024 * 1024)
        throw new Error("Document exceeds the 1 MiB editing limit.");
    return parse(source, {
        sourceType: "module",
        plugins: [
            ...(language.startsWith("typescript")
                ? ["typescript" as const]
                : []),
            ...(language.endsWith("react") || language === "javascript"
                ? ["jsx" as const]
                : []),
        ],
    });
    ...
    for (const node of tree.program.body) {
        ...
        for (const item of node.specifiers) {
            if (
                ...
                namespaces.push(item.local.name);
            if (
                item.type !== "ImportSpecifier" ||
                item.importKind === "type" ||
                item.importKind === "typeof"
            )
                continue;
            const name =
            ...
            if (
                ...
                (module === packageName || module === `${packageName}/${name}`)
            )
                values.set(item.local.name, name);
        }
    }
...
function nodes(
    ...
    while (stack.length) {
        if (++count > 200000)
            throw new Error("Document syntax exceeds supported limits.");
...
export function findIconAt(
    ...
    traverse(tree, {
        Identifier(location) {
            ...
            const binding = location.scope.getBinding(
                importDeclaration ? parent.node.local.name : node.name,
            );
            if (
                binding?.path.isImportSpecifier() &&
                (importDeclaration || location.isReferencedIdentifier())
            )
                name = values.get(binding.identifier.name);
        },
        MemberExpression(location) {
            ...
            if (
                ...
                namespaces.includes(object.name) &&
                iconNamePattern.test(property.name)
            )
                name = property.name;
        },
    });
...
export function planImport(
    ...
    const tree = syntax(source, language);
    const { values, namespaces } = imports(tree);
    for (const [binding, imported] of values)
        if (imported === name) return { offset: 0, text: "", binding };
    if (namespaces.length)
        return { offset: 0, text: "", binding: `${namespaces[0]}.${name}` };
    const occupied = new Set<string>();
    nodes(tree.program, (node) => {
    ...
    for (const directive of tree.program.directives)
        offset = Math.max(offset, directive.end ?? 0);
    for (const node of tree.program.body)
        if (node.type === "ImportDeclaration")
            offset = Math.max(offset, node.end ?? 0);
    const specifier = binding === name ? name : `${name} as ${binding}`;
    return {
        offset,
        binding,
        text: `${offset ? newline : ""}import { ${specifier} } from '${packageName}/${name}';${offset ? "" : newline}`,
    };
}
