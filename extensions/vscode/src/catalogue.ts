...
export async function readBounded(
    ...
    try {
        ...
        if (!stat.isFile() || stat.size > limit)
            throw new Error("Package file exceeds supported limits.");
...
async function ownedPath(root: string, relative: string): Promise<string> {
    ...
    if (!inside(root, actual))
        throw new Error("Package symlink escapes its root.");
    return actual;
}
export interface Catalogue {
    root: string;
    names: readonly string[];
    load(name: string): Promise<PreviewIcon>;
}
...
export async function resolveCatalogue(
    ...
    for (let depth = 0; depth < 64; depth++) {
        const candidate = path.join(
            directory,
            "node_modules/@bradleyhodges/sfsymbols",
        );
    ...
    throw new Error(
        "Install @bradleyhodges/sfsymbols in this project to preview icons.",
    );
}
async function catalogueAt(root: string): Promise<Catalogue> {
    const manifest: unknown = JSON.parse(
        await readBounded(await ownedPath(root, "package.json"), 262144),
    );
    if (
        !manifest ||
        typeof manifest !== "object" ||
        !("name" in manifest) ||
        manifest.name !== "@bradleyhodges/sfsymbols"
    )
        throw new Error("Invalid SF Symbols package manifest.");
    // Support the published conditional per-icon export contract, not arbitrary module layouts.
    const target = (
        manifest as {
            exports?: Record<string, { import?: { default?: unknown } }>;
        }
    ).exports?.["./sf*"]?.import?.default;
    if (
        typeof target !== "string" ||
        !/^\.\/(?:[A-Za-z0-9_-]+\/)+sf\*\.js$/.test(target)
    )
        throw new Error(
            "This package needs the supported ./sf* ESM export layout.",
        );
    ...
    for await (const entry of directory) {
        ...
        if (
            ...
            iconNamePattern.test(entry.name.slice(0, -3))
        ) {
            if (!entry.isFile())
                throw new Error("Icon leaves must be regular files.");
            names.push(entry.name.slice(0, -3));
            if (names.length > 20000)
                throw new Error("Package contains too many icons.");
        }
    }
    names.sort();
    ...
    return {
        ...
        names,
        async load(name) {
            if (!known.has(name) || !iconNamePattern.test(name))
