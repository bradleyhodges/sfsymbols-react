...
import { type Catalogue, resolveCatalogue } from "./catalogue";
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    const catalogues = new Map<
        string,
        { expires: number; value: Promise<Catalogue> }
    >();
    ...
    function catalogue(document: vscode.TextDocument): Promise<Catalogue> {
        if (document.uri.scheme !== "file")
            return Promise.reject(
                new Error("Save this document in a local project first."),
            );
        ...
        if (catalogues.size > 8)
            catalogues.delete(catalogues.keys().next().value!);
        void value.catch(() => {
            if (catalogues.get(key)?.value === value) catalogues.delete(key);
        });
        return value;
    }
                ...
                async provideHover(document, position, token) {
                    ...
                    try {
                        ...
                        markdown.appendCodeblock(
                            `import { ${name} } from '@bradleyhodges/sfsymbols/${name}';`,
                            "typescript",
                        );
    ...
    async function choose(icons: Catalogue): Promise<string | undefined> {
        ...
        picker.placeholder =
            "Search exact sf* names; select to preview, Enter to import";
        picker.matchOnDescription = true;
        picker.items = icons.names.map((label) => ({
            label,
            description: `@bradleyhodges/sfsymbols/${label}`,
        }));
        let sequence = 0;
        ...
        return new Promise((resolve) => {
            ...
            disposables.push(
                ...
                picker.onDidAccept(() =>
                    finish(picker.selectedItems[0]?.label),
                ),
                picker.onDidChangeActive((items) => {
                    ...
                    void (async () => {
                        ...
                        report(error);
                        if (!completed && request === sequence)
                            picker.title =
                                "SF Symbols — preview unavailable for this icon";
                    });
                }),
            );
            picker.show();
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand(
            "sfsymbols.insertIcon",
            async (requestedName?: unknown) => {
                ...
                if (
                    ...
                    void vscode.window.showInformationMessage(
                        "Open a JavaScript or TypeScript document first.",
                    );
                    return;
                }
                ...
                try {
                    ...
                    const name =
                        typeof requestedName === "string"
                            ...
                            : await choose(icons);
                    if (!name) return;
                    if (!icons.names.includes(name))
                        throw new Error(
                            "The selected icon is not in this installed package.",
                        );
                    if (
                        document.isClosed ||
                        document.version !== version ||
                        ...
                        void vscode.window.showInformationMessage(
                            "The document changed. Run SF Symbols: Preview and Import Icon again.",
                        );
                        return;
                    }
                    ...
                    if (edit.text) {
                        ...
                        if (!applied)
                            throw new Error(
                                "The document changed before the import could be applied.",
                            );
                    }
                    vscode.window.setStatusBarMessage(
                        `SF Symbols: use ${edit.binding}`,
                        5000,
                    );
                } catch (error) {
                    report(error);
                    void vscode.window.showWarningMessage(
                        error instanceof SyntaxError
                            ? "This document has unsupported or incomplete syntax. Finish the statement and try again."
                            : error instanceof Error
                              ? error.message
                              : "Unable to import this SF Symbol.",
                    );
                }
            },
        ),
