...
import { findIconAt, planImport } from "../src/imports";
import { escapeXml, parseIcon, renderSvg } from "../src/static-data";

const data =
    '{iconName:"sfCircle",viewBox:"0 0 20 20",svgPathData:[{d:"M0 0h20v20z",fillOpacity:.5}]}';
...
test("accepts exactly four SVG viewBox numbers separated by valid comma-whitespace", () => {
    const withViewBox = (viewBox: string) =>
        leaf.replace(
            'viewBox:"0 0 20 20"',
    ...
    for (const viewBox of [
        ...
        assert.throws(
            () => parseIcon(withViewBox(viewBox), "sfCircle"),
            /Invalid viewBox/,
            viewBox,
        );
    }
    for (const viewBox of [
        ...
        assert.equal(
            parseIcon(withViewBox(viewBox), "sfCircle").viewBox,
            "0 0 20 20",
            viewBox,
        );
    }
...
test("supports generated minified boolean metadata without allowing arbitrary unary expressions", () => {
    assert.equal(
        parseIcon(
            leaf.replace("iconName:", "keywords:[{generic:!0}],iconName:"),
            "sfCircle",
        ).name,
        "sfCircle",
    );
    assert.throws(() =>
        parseIcon(
            leaf.replace("iconName:", "keywords:[{generic:!call()}],iconName:"),
            "sfCircle",
        ),
    );
});
test("rejects executable or unsupported code anywhere, cycles and hostile SVG", () => {
    for (const source of [
        leaf + ";alert(1)",
        leaf.replace(data, "makeIcon()"),
        leaf.replace(data, '{get iconName(){return "sfCircle"}}'),
        leaf.replace("M0 0h20v20z", "<script>"),
        leaf.replace("20 20", "0 -1"),
        leaf.replace("fillOpacity:.5", 'fill:"url(https://evil)"'),
        "const a=b,b=a;export{a as sfCircle};",
        leaf.replace("const a=", "let a="),
    ])
        assert.throws(() => parseIcon(source, "sfCircle"));
...
test("recognizes TSX direct and root aliases and ignores strings and type-only imports", () => {
    ...
    assert.equal(
        findIconAt(source, source.lastIndexOf("ring"), "typescriptreact"),
        "sfCircle",
    );
    assert.equal(
        findIconAt(source, source.indexOf("'ring'") + 1, "typescriptreact"),
        undefined,
    );
    assert.equal(
        findIconAt(source, source.indexOf("as T") + 3, "typescriptreact"),
        undefined,
    );
});
test("hover recognizes both tokens of aliased value imports and excludes type-only imported tokens", () => {
    for (const module of [
        ...
        "@bradleyhodges/sfsymbols/sfCircle",
    ]) {
        const source = `import {sfCircle as ring} from '${module}';`;
        assert.equal(
            findIconAt(source, source.indexOf("sfCircle"), "typescript"),
            "sfCircle",
        );
        assert.equal(
            findIconAt(source, source.indexOf("ring"), "typescript"),
            "sfCircle",
        );
    }
    const collision = `import {sfCircle as ring, sfSquare as sfCircle} from '@bradleyhodges/sfsymbols';`;
    assert.equal(
        findIconAt(collision, collision.indexOf("sfCircle"), "typescript"),
        "sfCircle",
    );
    for (const clause of [
        "type {sfCircle as ring}",
        "{type sfCircle as ring}",
    ]) {
...
test("hover excludes shadowed parameters and unrelated object keys", () => {
    ...
    assert.equal(
        findIconAt(source, source.indexOf("return ring") + 7, "typescript"),
        undefined,
    );
    assert.equal(
        findIconAt(source, source.lastIndexOf("ring"), "typescript"),
        undefined,
    );
});
test("rejects malformed SVG commands and nonfinite path coordinates", () => {
    for (const d of ["Mz", "M0 0L1", "M0 0A1 1 0 4 0 2 2", "M1e999 0"])
        assert.throws(() =>
            parseIcon(leaf.replace("M0 0h20v20z", d), "sfCircle"),
        );
});
test("preserves empty non-rendering paths present in the current corpus", () => {
    assert.equal(
        parseIcon(leaf.replace("M0 0h20v20z", ""), "sfCircle").paths[0].d,
        "",
    );
});
...
test("imports preserve alias/namespace duplicates and type-only collisions", () => {
    assert.equal(
        planImport(
            `import {sfCircle as ring} from '@bradleyhodges/sfsymbols';`,
            "sfCircle",
            "typescript",
        ).binding,
        "ring",
    ...
    assert.equal(
        planImport(
            `import * as icons from '@bradleyhodges/sfsymbols';`,
            "sfCircle",
            "typescript",
        ).binding,
        "icons.sfCircle",
    );
    const edit = planImport(
        `import type {sfCircle} from '@bradleyhodges/sfsymbols';`,
        "sfCircle",
        "typescript",
    );
    ...
    assert.match(
        edit.text,
        /import \{ sfCircle as sfCircle2 \} from '@bradleyhodges\/sfsymbols\/sfCircle';/,
    );
});
...
test("resolves installed pnpm-style package and rejects escaping manifest targets", async () => {
    ...
    try {
        ...
        await mkdir(path.join(root, "node_modules/@bradleyhodges"), {
            recursive: true,
        });
        await symlink(
            pkg,
            path.join(root, "node_modules/@bradleyhodges/sfsymbols"),
            "junction",
        );
        const manifest = {
            name: "@bradleyhodges/sfsymbols",
            exports: {
                "./sf*": { import: { default: "./dist/module/sf*.js" } },
            },
        };
        await writeFile(
