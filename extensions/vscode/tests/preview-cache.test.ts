...
test('failed eviction never writes beyond the 128-file cache bound', async () => {
  ...
  const loadedModule: LoadedModule = { exports: {} as LoadedModule['exports'] };
  const load = vm.runInThisContext(
    `(function(require,module,exports){${result.outputFiles[0].text}\n})`,
  ) as (loader: (name: string) => unknown, module: LoadedModule, exports: LoadedModule['exports']) => void;
  load(name => name === 'vscode' ? vscode : require(name), loadedModule, loadedModule.exports);
  const cache = new loadedModule.exports.PreviewCache({ fsPath: 'memory-only' });
  let rejected = 0;
