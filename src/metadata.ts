 ...
 */
export function getIconKeywords(icon: unknown): readonly SFIconKeyword[] {
    const keywords = ownValue(icon, "keywords");
    const result: SFIconKeyword[] = [];
    if (Array.isArray(keywords)) {
        for (let index = 0; index < keywords.length; index++) {
            const keyword = ownValue(keywords, String(index));
            if (!isRecord(keyword)) continue;
        ...
        for (const key of Object.keys(keywords)) {
            ...
            if (typeof text === "string") result.push(Object.freeze({ text }));
        }
    }
...
export function getIconVariants<CustomVariant extends string = never>(
    ...
    return Object.freeze(result) as Readonly<
        Partial<Record<SFIconVariant | CustomVariant, string>>
    >;
}
